...
$conn = mysqli_connect('localhost','root','Ss123456','mark') or die('connection failed');
...
if(isset($_POST['name'])){
   ...
   $select_message = mysqli_query($conn, "SELECT * FROM `contact_for` WHERE name = '$name' AND email = '$email'  AND message = '$msg'") or die('query failed');

   if(mysqli_num_rows($select_message) > 0){
      $message[] = 'Message Sent Already!';
   }else{
      mysqli_query($conn, "INSERT INTO `contact_for`(name, email, message) VALUES('$name', '$email', '$msg')") or die('query failed');
